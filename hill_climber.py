"""Hill climbing utilities for optimizing GPac Pac-Man controllers."""
...
def hill_climb_run(
    ...
    *,
    ...
    ----------
    num_evaluations:
        Total number of controller evaluations to perform. The initial
        random controller counts toward this budget.
    config:
        Configuration mapping created by ``read_config`` in the notebook.
        The dictionary must expose the ``pac_init`` and ``game`` sections.
        An optional ``hill_climb`` section may override the defaults used
        by this implementation (see below).
    rng:
        Optional random number generator. When omitted the global
        ``random`` module is used which preserves the existing behaviour in
        the notebook.

    Hill-climb configuration options
    --------------------------------
    ``hill_climb.seed``
        Seed applied to both the supplied ``rng`` as well as the global
        ``random`` module used by ``play_GPac``. This makes runs
        reproducible without forcing callers to seed manually.
    ``hill_climb.restart_interval``
        Optional number of evaluations between forced restarts. When
        positive the algorithm periodically discards the current solution
        and restarts from a fresh random controller.
    ``hill_climb.constant_sigma``
        Standard deviation used when perturbing constants inside the tree.
        Defaults to ``0.5``.
    ``hill_climb.max_mutation_depth``
        Upper bound on the depth of subtrees generated during mutation.
        The value is clamped to the ``depth_limit`` of the genotype.

    Returns
    -------
    HillClimbResult
        Dataclass containing the overall best solution along with the
        artefacts requested in the assignment (log, stairstep data and
        histogram).
    """
